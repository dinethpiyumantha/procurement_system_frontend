  ...
  <div>
    ...
    >
      <span slot="action" slot-scope="record">
        ...
        <a-button type="primary" size="small" style="margin-left:10px" @click="paymentCompleter(record,'Approved')">Payment Complete</a-button>
      </span>
    </a-table>
...
export default {
  ...
  created() {
    ...
    axios
      .get("http://127.0.0.1:8000/api/purchasedOrders/allApprovedOrders")
      .then((response) => {
  ...
  methods: {
    ...
    paymentCompleter(record,approved) {
      console.log(record.id);
      this.$http.put('http://127.0.0.1:8000/api/purchasedOrders/approve/'+record.id,{ 'payment_status' : approved})
    },
    deleteOrder(record) {
      console.log(record.id);
    },
